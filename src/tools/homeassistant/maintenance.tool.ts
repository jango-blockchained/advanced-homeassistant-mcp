...
interface LightUsageAnalysis {
    ...
    never_used: Array<{ entity_id: string; friendly_name: string; last_changed?: string; days_inactive: number | string }>;
    high_usage: unknown[];
    ...
    async findOrphanedDevices(): Promise<OrphanedDevice[]> {
        try {
            ...
            const orphanedDevices = states.filter(state => {
                ...
                const notUpdatedRecently = (state.last_updated ?? '').length > 0 && 
            ...
            return orphanedDevices.map(device => ({
                ...
                reason: device.state === 'unavailable' ? 'unavailable' : 
            ...
            for (const state of states) {
                ...
                if (state.state === 'unavailable') {
                    ...
                    health.offline_devices.push({
                        ...
                        friendly_name: (state.attributes.friendly_name as string | undefined) ?? state.entity_id,
                        domain: domain,
                    });
...
async function executeMaintenanceLogic(params: MaintenanceParams): Promise<string> {
    ...
    switch (params.action) {
        ...
        case "cleanup_orphaned_entities": {
            ...
            return JSON.stringify({
                action: params.action,
                warning: "Automatic cleanup not yet implemented. Please remove entities manually through Home Assistant UI.",
