...
const smartScenariosSchema = z.object({
    ...
    mode: z.enum(["detect", "apply", "auto"]).optional().default("detect")
        .describe("detect=only report, apply=execute actions, auto=create automation"),
    
    rooms: z.array(z.string()).optional()
        .describe("Specific rooms/areas to apply scenario to"),
    
    temperature_reduction: z.number().min(1).max(10).optional().default(3)
...
class SmartScenariosService {
    ...
    async detectNobodyHome(): Promise<ScenarioDetection> {
        try {
            ...
            const presenceSensors = states.filter(s => 
                ...
                s.entity_id.includes('occupancy')
            );
            ...
            const climateActive = states.filter(s => 
    ...
    private isInRooms(entity: { entity_id: string; attributes: Record<string, unknown> }, rooms: string[]): boolean {
        const area = ((entity.attributes.area_id as string | undefined) ?? '') || this.extractArea(entity.entity_id);
        return rooms.some(room => area.toLowerCase().includes(room.toLowerCase()));
    }

    // Automation generation helpers
    private generateNobodyHomeAutomation(lights: Array<{ entity_id: string }>, climate: Array<{ entity_id: string }>): Record<string, unknown> {
        return {
    ...
    private generateWindowHeatingAutomation(climate: { entity_id: string }, windows: Array<{ entity_id: string }>): Record<string, unknown> {
        return {
    ...
    private generateDaylightAutomation(lights: Array<{ entity_id: string }>): Record<string, unknown> {
        return {
