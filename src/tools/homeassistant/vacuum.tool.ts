...
const vacuumControlSchema = z.object({
    ...
    entity_id: z.string().optional().describe("The entity ID of the vacuum (required for most actions)"),
...
async function execute(params: VacuumControlInput): Promise<string> {
    ...
    try {
        switch (action) {
            ...
            case "set_fan_speed": {
                if (!entity_id) {
                    return JSON.stringify({ success: false, error: "entity_id is required for set_fan_speed action" });
                }
                if (!fan_speed) {
                    return JSON.stringify({ success: false, error: "fan_speed is required for set_fan_speed action" });
                }
                const success = await haVacuumService.callService("set_fan_speed", entity_id, { fan_speed });
                return JSON.stringify({
                    success,
                    message: success ? `Successfully set fan speed to ${fan_speed} on ${entity_id}` : `Failed to set fan speed on ${entity_id}`
                });
            }

            case "send_command": {
                if (!entity_id) {
                    return JSON.stringify({ success: false, error: "entity_id is required for send_command action" });
                }
                if (!command) {
                    return JSON.stringify({ success: false, error: "command is required for send_command action" });
                }
                ...
                return JSON.stringify({
                    success,
                    message: success ? `Successfully sent command ${command} to ${entity_id}` : `Failed to send command to ${entity_id}`
                });
            }
