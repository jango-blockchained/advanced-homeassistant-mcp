...
describe("TokenManager", () => {
  ...
  const validToken =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiZXhwIjoxNjE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";
  ...
  describe("Token Encryption/Decryption", () => {
    ...
    test("should fail decryption with wrong key", () => {
      const encrypted = TokenManager.encryptToken(validToken, encryptionKey);
      expect(() =>
        TokenManager.decryptToken(encrypted, "wrong-key-32-chars-long!!!!!!!!"),
      ).toThrow();
    });
  });

  describe("Token Validation", () => {
    test("should validate correct tokens", async () => {
      const payload = {
        sub: "123",
        name: "Test User",
    ...
    test("should handle tokens with missing expiration", async () => {
    ...
    test("should handle undefined and null inputs", async () => {
      const undefinedResult = await TokenManager.validateToken(undefined);
      expect(undefinedResult.valid).toBe(false);
      expect(undefinedResult.error).toBe("Invalid token format");

      const nullResult = await TokenManager.validateToken(null);
  ...
  describe("Error Handling", () => {
    test("should throw descriptive errors for invalid inputs", () => {
      ...
      expect(() => TokenManager.encryptToken(null as unknown as string, encryptionKey)).toThrow(
        "Invalid token",
      );
      expect(() => TokenManager.encryptToken(validToken, null as unknown as string)).toThrow(
        "Invalid encryption key",
      );
      expect(() => TokenManager.decryptToken("invalid-base64", encryptionKey)).toThrow(
